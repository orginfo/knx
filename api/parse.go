...
func parseListOfInt(value string) (values []int64, err error) {
	if len(value) == 0 {
		return
	}
...
// parseFloatName
func parseFloatName(str string) (result map[float64]string, err error) {
...
// parseIntName
func parseIntName(str string) (result map[int64]string, err error) {
	var values map[string]string
	values, err = parseValueName(str)
	if err != nil {
		return
